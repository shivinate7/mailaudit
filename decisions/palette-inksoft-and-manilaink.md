...
The `inkSoft` token was darkened from an earlier value that failed its contrast check, and it is the most used token after `ink`. The `manilaInk` token is checked against the `manila` background it actually sits on, not against the card surface.
