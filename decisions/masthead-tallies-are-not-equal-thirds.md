...
The three tally cells, for cards, packages, and value, are not equal thirds. The value figure is much wider than the two counts, so it needed more room. The value cell now gets a larger flex share, and all three cells share the same headroom. `money()` still stays the exact format wherever a figure must read precisely.
